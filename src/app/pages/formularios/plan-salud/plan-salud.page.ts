  ...
  public async Submit() {   
    ...
    if (this.formPlanSalud.valid) {
        try {
            ...
            localStorage.setItem('Poliza_Emitida', JSON.stringify(response));
            
            localStorage.setItem('Numero_Poliza', JSON.stringify(this.currentPolizaNumber));
            ...
            if (response.status === true) {
              ...
              } else if(response.message === "El asegurado/titular no cumple con los criterios de edad para este plan. (Min: 0  , Max: 74 )."){
                this.toastMessage('El asegurado/titular no cumple con los criterios de edad para este plan. (Min: 0  , Max: 74 ).','warning')
              }
            ...
            if (err instanceof HttpErrorResponse) {
              ...
              if(err.status === 500  && err.error.message === "El asegurado/titular no cumple con los criterios de edad para este plan. (Min: 0  , Max: 74 ).")
              {
                this.toastMessage('El asegurado/titular no cumple con los criterios de edad para este plan. (Min: 0  , Max: 74 )','warning')
              }
            }
...
private async registerPayment(data:any) {
  try {
