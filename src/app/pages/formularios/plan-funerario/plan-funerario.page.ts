...
export class PlanFunerarioPage implements OnInit, AfterViewChecked {
  ...
  constructor(private changeDftc: ChangeDetectorRef) {
    this.generateForm();
    this.formPlanSalud
      .get('dec_term_y_cod')
  ...
  getNextPolizaNumber(): number {
    ...
    this.currentPolizaNumber = numberFromStorage
      ? parseInt(numberFromStorage, 10)
      : 1;
    ...
    localStorage.setItem(
      'currentPolizaNumber',
      this.currentPolizaNumber.toString()
    );
  ...
  selectEstadoT(estado: Estado): void {
    ...
    this.filteredCiudades = this.ciudad.filter(
      (ciudad) => ciudad.cestado === estado.cestado
    );
  }
  ...
  onCiudadT(event: Event): void {
    ...
    const estado = this.estados.find(
      (e) => e.cestado === Number(estadoSeleccionado)
    );
  ...
  selectCiudadT(ciudad: Ciudad): void {
    ...
    const inputElement = document.querySelector(
      'input[formControlName="ciudad_tomador"]'
    ) as HTMLInputElement;
    if (inputElement) {
  ...
  selectEstadoTI(estado: Estado): void {
    ...
    const inputElement = document.querySelector(
      'input[formControlName="estado_titular"]'
    ) as HTMLInputElement;
    if (inputElement) {
      inputElement.value = estado.xdescripcion_l;
    }
    ...
    // Filtrar ciudades por el estado seleccionado
    this.filteredCiudades2 = this.ciudad.filter(
      (ciudad) => ciudad.cestado === estado.cestado
    );
  }
  ...
  selectCiudadTI(ciudad: Ciudad): void {
    ...
    const inputElement = document.querySelector(
      'input[formControlName="ciudad_titular"]'
    ) as HTMLInputElement;
    if (inputElement) {
      inputElement.value = ciudad.xdescripcion_l;
    }
  ...
  private generateForm() {
    ...
    this.formPlanSalud = this.fb.group({
      ...
      correo_tomador: new FormControl('', [
        Validators.required,
        Validators.email,
      ]),
      ...
      correo_titular: new FormControl('', [
        Validators.required,
        Validators.email,
      ]),
    ...
    this.tomadorSubscription.add(
      this.formPlanSalud
        .get('apellido_tomador')!
        .valueChanges.subscribe((value) => {
          if (this.isTitularDisabled) {
            this.formPlanSalud
              .get('apellido_titular')!
              .setValue(value, { emitEvent: false });
          }
        })
    );
    ...
    this.tomadorSubscription.add(
      this.formPlanSalud
        .get('estado_civil_tomador')!
        .valueChanges.subscribe((value) => {
          if (this.isTitularDisabled) {
            this.formPlanSalud
              .get('estado_civil_titular')!
              .setValue(value, { emitEvent: false });
          }
        })
    );
    ...
    this.tomadorSubscription.add(
      this.formPlanSalud
        ...
        .valueChanges.subscribe((value) => {
          if (this.isTitularDisabled) {
            const estado = this.estados.find(
              (e) => e.cestado.toString() === value
            );
            if (estado) {
              this.formPlanSalud
                .get('estado_titular')!
                .setValue(estado.cestado.toString(), { emitEvent: false });
    ...
    this.tomadorSubscription.add(
      this.formPlanSalud
        .get('direccion_tomador')!
        .valueChanges.subscribe((value) => {
          if (this.isTitularDisabled) {
            this.formPlanSalud
              .get('direccion_titular')!
              .setValue(value, { emitEvent: false });
          }
        })
    );
    ...
    this.tomadorSubscription.add(
      this.formPlanSalud
        .get('telefono_tomador')!
        .valueChanges.subscribe((value) => {
          if (this.isTitularDisabled) {
            this.formPlanSalud
              .get('telefono_titular')!
              .setValue(value, { emitEvent: false });
          }
        })
    );
  ...
  private async getNumberPoliza() {
    try {
      const response: any = await (
        await this.polziaService.getNumber()
      ).toPromise();
  ...
  private async updateNumberPoliza() {
    try {
      ...
      const response: any = await (
        await this.polziaService.updateNumber(datos)
      ).toPromise();
  ...
  private storageData() {
    const StoredCedula: string | null = localStorage.getItem(
      'documento_identidad'
    );
    ...
    if (this.formPlanSalud.valid) {
      try {
        const response = await (
          await this.polziaService.emisionMundial(this.formPlanSalud.value)
        ).toPromise();
        ...
        if (response.status === true) {
          ...
          setTimeout(() => {
            this.savePoliza({
              ...
              titular_apellido:
                this.formPlanSalud.get('apellido_tomador')?.value,
            });
          }, 1000);
  ...
  public getInfoData() {
    return this.http
      .get<any>(`${environment.mailPoliza}/data`)
      .subscribe((data: any) => {
